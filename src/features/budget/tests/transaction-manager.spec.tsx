...
describe('TransactionManager', () => {
  ...
  describe('Unauthenticated state', () => {
    ...
    test('shows welcome message and login button when unauthenticated', () => {
      render(<TransactionManager />);

      expect(screen.getByText('Personal Budget Manager')).toBeInTheDocument();
      expect(screen.getByText('Welcome!')).toBeInTheDocument();
      expect(
        screen.getByText(/please sign in to access your personal budget manager/i),
      ).toBeInTheDocument();
      expect(screen.getByRole('button', { name: /sign in with google/i })).toBeInTheDocument();

      // Should not show transaction list or form
  ...
  describe('Authenticated state', () => {
    ...
    test('renders both form and list sections when authenticated', async () => {
      render(<TransactionManager />);
  ...
  test('handles 401 errors appropriately', async () => {
    ...
    // Should show login prompt, not error message
    expect(screen.getByText('Welcome!')).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /sign in with google/i })).toBeInTheDocument();
  });

  test('displays user menu with correct user information', async () => {
    mockUseSession.mockReturnValue({
      data: createMockSession({
        id: 'test-user-id',
        email: 'test@example.com',
